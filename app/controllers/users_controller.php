...
class UsersController extends AppController {
  ...
  /**
   * Displays the user profile page. This method doesn't requirean id parameter
   * because we only want the user to edit his/her own profile. The Auth->user
   * id is used in lieu of a method param.
   *
   * @access	public
   */
  public function edit() {
    ...
    if( !empty( $this->data ) ) {
      ...
      # Do a little extra if the user intends to change their password.
      if( !empty( $this->data['User']['password'] ) ) {
        # Password is hashed by the Auth component. Check for an empty hash.
        # If the hash isn't empty, hash the confirmation for comparison
      ...
      if( $this->User->save( $this->data ) ) {
        $this->Session->setFlash( __( 'Your profile has been updated.', true ), null, null, 'success' );
        $this->redirect( array( 'action' => 'edit' ), null, true );
      }
      else {
        $this->Session->setFlash( __( 'There was a problem updating your profile. Please check for errors below.', true ), null, null, 'validation' );
      }
    }
    else {
