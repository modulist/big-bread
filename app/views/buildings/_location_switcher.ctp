  ...
  <ul class="other-location-list">
    <?php foreach( $other_locations as $i => $other_location ): ?>
      ...
      <li class="leaf <?php echo join( ' ', array_filter( $classes ) ) ?>">
        <div class="other-location-name"><?php echo $this->Html->link( !empty( $other_location['Building']['name'] ) ? h( $other_location['Building']['name'] ) : h( $other_location['Address']['address_1'] ), array( 'controller' => $this->name, 'action' => $this->action, $other_location['Building']['id'] ) ) ?></div>
